...
function differenceEvenOddWorker(...arr) {//Пишем функцию differenceEvenOddWorker вычисления разницы сумм чётных и нечётных элементов
  ...
  for (let i = 0; i < arr.length; i++) {//Цикл для перебора массива на увеличение
    if (arr[i] % 2 === 0) {// Берем остаток, если он абсолютно равен 0, то это четный элемент
      sumEvenElement += arr[i]; //Если элемент четный, то прибавляем текущий элемент
    } else {
      sumOddElement += arr[i]; //В противном случае выводим нечетные то прибавляем текуший элемент
    }
  }
}
...
function averageEvenElementsWorker(...arr) { //Пишем функцию averageEvenElementsWorker,
                                             // которая будет вычислять среднее значение чётных элементов
  if (arr.length === 0) {//При длине массива абсолютно раное 0 возвращаем 0
    return 0;
  }
  
  let sumEvenElement = 0;//присваиваем изначальное значение для суммы, т.к. сумма с 0 никак на результат не повлияет
  let countEvenElement = 0;// здесь также, начальное значение для переменной-счётчика. Если просто создать переменную, 
                           //без присвоения значения, то операция + приведёт к ошибочному результату - NaN

  for (let i = 0; i < arr.length; i++) { //Цикл для перебора массива на увеличение
    if (arr[i] % 2 === 0) { //Применяем оператор нахождения остатка для нахождения четного элемента
      sumEvenElement += arr[i]; //Если элемент четный прибавляем текущий элемент
      countEvenElement +=1; // Если элемент нечетный прибавляем текущий элемент
                            // +=1ж это арифметика с приращением, т.е. к имеющемуся в этой переменной значению прибавляем один.
    }
  }
