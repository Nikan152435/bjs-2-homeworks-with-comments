...
// в аргументы конструктора передается то, что будет установлено при создании экземпляра класса
    constructor(name, releaseDate, pagesCount) {
// super - вызываем конструктор родителя, передаем те аргументы, которые там поименованы
        super(name, releaseDate, pagesCount);// Вызов родительского поведения.переопределяет методы с исп. родительского 
                                             //поведения.
        this.type = 'magazine';// устанавливается/присваивается/переопределяется значение в свойство, которое будет  
                               //только в  экземплярах этого класса, оно же уже создано по умолчанию.
    }
} 
